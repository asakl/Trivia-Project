...
void Communicator::startThreadForNewClient()
{
	...
	try
	{
		...
		//msg on server
		cout << __FUNCTION__": Client accepted. Server and client can speak" << endl;
...
void Communicator::bindAndListen()
{
	...
	while (true)
	{
		cout << __FUNCTION__": Waiting for client connection request" << endl;
...
void Communicator::handleRequests(SOCKET client_socket)
{
	...
	while (currentRequest.id != SIGNOUT_REQUEST)
	{
		...
		{
			cout << client_socket << ": Client " << client_socket << " unexpectedly closed the socket" << endl;
			return;
		}
		...
		if (currentRequest.id == SIGNOUT_ID)
		{
			...
			cout << client_socket<< ": Finished communicating with client " << client_socket << endl;
			break;
		}

		//Get the relevant handler for this request.
